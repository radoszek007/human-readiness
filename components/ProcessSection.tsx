        ...
        <div className="mt-12 grid gap-5 lg:grid-cols-4">
          {processSteps.map((step, index) => (
            <article key={step.title} className="relative rounded-[1.25rem] border border-slate-200 bg-white p-6 shadow-soft">
              <div className="mb-8 flex h-12 w-12 items-center justify-center rounded-full bg-accent text-sm font-black text-white">
                {String(index + 1).padStart(2, "0")}
              </div>
              <h3 className="font-heading text-xl font-extrabold text-dark">{step.title}</h3>
