    ...
    <section className="bg-bg py-20 text-white sm:py-24">
      <div className="section-shell">
        ...
        <div className="mt-12 max-w-4xl">
          <h3 className="font-heading text-2xl font-bold leading-tight text-white sm:text-3xl">Rozdíl je v nácviku</h3>
        ...
        <div className="mt-12 overflow-hidden rounded-[1.25rem] border border-border/40 bg-panel-dark/70 shadow-panel">
          <div className="hidden bg-dark/20 text-sm font-bold uppercase tracking-[0.14em] text-text-muted md:grid md:grid-cols-[0.72fr_1fr_1fr]">
          ...
          {comparisonRows.map((row) => (
            <div key={row.area} className="grid grid-cols-1 border-t border-border/40 md:grid-cols-[0.72fr_1fr_1fr]">
              <div className="bg-white/5 p-4 font-body text-lg font-bold sm:p-5">{row.area}</div>
              <div className="p-4 leading-7 text-text-muted sm:p-5">
                <span className="mb-2 block text-xs font-bold uppercase tracking-[0.14em] text-text-secondary md:hidden">
                  Běžné školení
                </span>
                {row.common}
              </div>
              <div className="border-border/40 p-4 leading-7 text-text-primary md:border-l sm:p-5">
                <span className="mb-2 block text-xs font-bold uppercase tracking-[0.14em] text-accent-soft md:hidden">
                  Resilium
                </span>
                {row.resilium}
              </div>
