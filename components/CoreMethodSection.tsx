        ...
        <div className="mt-12 grid gap-4 lg:grid-cols-4">
          {coreSteps.map((step) => (
            <article key={step.letter} className="relative overflow-hidden rounded-[1.25rem] border border-slate-200 bg-white p-6 shadow-soft">
              <div className="absolute right-4 top-2 font-heading text-8xl font-black leading-none text-bg/10">{step.letter}</div>
              <div className="relative">
                <span className="flex h-14 w-14 items-center justify-center rounded-2xl bg-bg text-2xl font-black text-white">
                  {step.letter}
                </span>
                <h3 className="mt-5 font-heading text-2xl font-extrabold text-dark">{step.label}</h3>
